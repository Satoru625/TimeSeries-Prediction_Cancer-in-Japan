...
d <- read.csv("cancer/input/cancer_mortality_jpn.csv",check.names = F)
...
df_liver <- ts(df[df$code==8,"adj_count"],start = 1958, frequency = 1)
df_lymph <- ts(df[df$code==25,"adj_count"],start = 1958, frequency = 1)
df_gast <- ts(df[df$code==67,"adj_count"],start = 1958, frequency = 1)
df_bil <- ts(df[df$code==9,"adj_count"],start = 1958, frequency = 1)
...
# train
df_train <- filter(df, year<"2010-01-01")
df_train_liver <- ts(df_train[df_train$code==8,"adj_count"],start = 1958, frequency = 1)
df_train_lymph <- ts(df_train[df_train$code==25,"adj_count"],start = 1958, frequency = 1)
df_train_gast <- ts(df_train[df_train$code==67,"adj_count"],start = 1958, frequency = 1)
df_train_bil <- ts(df_train[df_train$code==9,"adj_count"],start = 1958, frequency = 1)

# pred
train.liver <- auto.arima(df_train_liver,ic="aic",stepwise = F,approximation = F)
autoplot(df_liver) + autolayer(train.liver$fitted) + autolayer(forecast(train.liver,h=11),series = "ARIMA",alpha=.5)
train.lymph <- auto.arima(df_train_lymph,ic="aic",stepwise = F,approximation = F)
autoplot(df_lymph) + autolayer(train.lymph$fitted) + autolayer(forecast(train.lymph,h=11),series = "ARIMA",alpha=.5)
train.gast <- auto.arima(df_train_gast,ic="aic",stepwise = F,approximation = F)
autoplot(df_gast) + autolayer(train.gast$fitted) + autolayer(forecast(train.gast,h=11),series = "ARIMA",alpha=.5)
train.bil <- auto.arima(df_train_bil,ic="aic",stepwise = F,approximation = F)
autoplot(df_bil) + autolayer(train.bil$fitted) + autolayer(forecast(train.bil,h=11),series = "ARIMA",alpha=.5)
